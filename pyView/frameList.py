...
class SavedFrames(QHBoxLayout):
    ...
    def update_images(self):
        images = [f for f in listdir(self.PATH_TO_FRAMES) if isfile(join(self.PATH_TO_FRAMES, f))]
    ...
    def __add_image(self,image):
        if(image not in self.saved_items):
            self.saved_items[image] = savedFrame(self.PATH_TO_FRAMES,image,self.display)
